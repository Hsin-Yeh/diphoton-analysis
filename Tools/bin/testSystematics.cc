...
int main(int argc, char *argv[])
{
  ...
  testTheSystematics(year, inputworkspaces, outputworkspaces, thecoupling);
...
void testTheSystematics(const std::string &year, const std::string &ws_indir, const std::string &ws_outdir, const std::string &couplingIn){
  ...
  for(auto isample : samples) {
    ...
    tmpin.fparamshape = TFile::Open(Form("%s/SignalParametricShapes_ws_%s.root", ws_indir.c_str(), couplingIn.c_str()));
