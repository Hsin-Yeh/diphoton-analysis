...
//----------------------------
//2017
float MAXmass = 6000.;//3000.//6000.
float MAXmassBE = 6000.;//2500.//6000.
//In the full range (e.g. 500,3000) will have 20 GeV bins same as in the past. 
//In the full range (e.g. 500,3000) will have 2 GeV. 
//In the full range (e.g. 500,10000) will have 5 GeV. 
int nBinsMass = 1100;//150//1250//1100//275
int nBinsMassBE = 1100;//125//1000//1100//275
float nBinsMassperGeV = 5.;
//----------------------------
//2018
// float MAXmass = 2500.;
// float MAXmassBE = 2700.;
// //In the full range will have 20 GeV bins same as in the past. 
// int nBinsMass = 125;
// int nBinsMassBE = 135;
//----------------------------
float minMassForFitBkg = 500.;
float maxMassForFitBkg = 1300.;
...
void AddBkgData(RooWorkspace* w, const std::string &isample, const std::string &year, const std::string &ws_dir, bool blind, std::vector<std::string> cats, int thecat);
...
void runAllFits(const std::string &year, const std::string &ws_dir, std::vector<std::string> cats, const std::string &themodel, int order, int thecat);
void runfits(const std::string &year, const std::string &ws_dir, const std::string &isample, std::vector<std::string> cats, const std::string &themodel, int order, int thecat);
std::string getBase(const std::string & sampleName);
void SetConstantParams(const RooArgSet* params) ;
void RemoveRangeParams(const RooArgSet* params) ;
...
{
  std::string year, inputdir, outputdir, themodel, order, thecat;
  

  if(argc!=7) {
    std::cout << "Syntax: bkgFit.exe [2016/2017/2018] [input] [output] [themodel] [order] [thecat]" << std::endl;
      return -1;
  }
  else {
    year = argv[1];
  ...
  //========================================================================
  //Run the fits
  runAllFits(year,outputdir,cats,themodel,std::stoi(order),std::stoi(thecat));
...
void runAllFits(const std::string &year, const std::string &ws_dir, std::vector<std::string> cats, const std::string &themodel, int order, int thecat){
  ...
  for(auto isample : samples) {
    ...
    runfits(year, ws_dir, isample, cats, themodel, order, thecat);
  }
...
void AddBkgData(RooWorkspace* w, const std::string &isample, const std::string &year, const std::string &ws_dir, bool blind, std::vector<std::string> cats, int thecat) {
  ...
  //NO -> we want to see the whole range. Later we will limit the fit to our ranges of interest. 
  minMassFit = MINmass;
  //MAXMASS is the greatest for BB so we are ok. 
  // maxMassFit = std::max(MAXmass,MAXmassBE);
  maxMassFit = MAXmass;
    
  RooRealVar* mgg = buildRooVar("mgg", "M(gg)", nBinsMass, MINmass, MAXmass,"GeV");
  RooRealVar* setBins = new RooRealVar("setBins", "setBins", nBinsMass);
  ...
  nBinsMass=(int) round( (maxMassFit-minMassFit)/nBinsMassperGeV );
  ...
  ctmp->SetLogy();
  ctmp->SaveAs( Form("%s/BkgOnlyData_%s_%s_log.png", ws_dir.c_str(),cats[thecat].c_str(),year.c_str()) );
  ctmp->SetLogy(0);
  ctmp->SaveAs( Form("%s/BkgOnlyData_%s_%s.png", ws_dir.c_str(),cats[thecat].c_str(),year.c_str()) );
...
void runfits(const std::string &year, const std::string &ws_dir, const std::string &isample, std::vector<std::string> cats, const std::string &themodel, int order, int thecat){
  ...
  // Better run them with the push_back(themodel) above. I don't use the ones below any more. 
  // 2017: Chosen ones (No diff between 5 and 20 GeV bins, so go with 5 GeV as in the past. )
  // if ( year == "2017"){
  //   models.push_back("Laurent"); // 5R: N, 5NoR: Y                               // 20R: N, 20NoR: Y
  //   models.push_back("Exponential");  //5R: N (only EBEE), 5NoR: Y                // 20R: N (only EBEE), 20NoR: Y
  //   models.push_back("Expow");  //5R: N (only EBEB), 5NoR: N (only EBEE)          // 20R: N (only EBEB), 20NoR: N (only EBEE)
  // }

  // // 2018: Chosen ones
  // if ( year == "2018"){
  //   models.push_back("Laurent"); //5R: N, 5NoR: Y                               // 20R: , 20NoR: 
  //   models.push_back("Exponential");  //5R: Y, 5NoR: Y                             // 20R:  , 20NoR: 
  //   models.push_back("invpow");  //5R: Y, 5NoR: Y                               // 20R:  , 20NoR: 

  // }

  //   models.push_back("Expow");  //5R: Y, 5NoR: N                                   // 20R:  , 20NoR: 
  // models.push_back("Atlas");  //5R: N (only EBEE), 5NoR:  N (only EBEE)      // 20R: N (only EBEE), 20NoR: N (only EBEE)
  // models.push_back("PowerLaw"); 5R: N, 5NoR: N                               // 20R: N, 20NoR: N
  // models.push_back("invpow");  5R: N (only EBEE), 5NoR:  N (only EBEE)       // 20R: N (only EBEE), 20NoR: N (only EBEE)
  // models.push_back("invpowlin");  5R: N (only EBEE), 5NoR:  N (only EBEE)    // 20R: N (only EBEE), 20NoR: N (only EBEE)

  // models.push_back("PowerLaw"); 5R: N, 5NoR: N                              // 20R: , 20NoR: 
  // models.push_back("Atlas");  5R: N, 5NoR: N                                   // 20R:  , 20NoR: 
  // models.push_back("invpowlin");  5R: N, 5NoR: N                             // 20R:  , 20NoR: 
  ...
  modelorder["pow"].push_back(1);//1
  modelorder["pow"].push_back(1);//3

  modelorder["Laurent"].push_back(1);//1
  modelorder["Laurent"].push_back(2);//2
  ...
  modelorder["invpow"].push_back(1);//1 
  modelorder["invpow"].push_back(1);//1

  modelorder["invpowlin"].push_back(1);//1 No good fit
  modelorder["invpowlin"].push_back(1);//1 

  modelorder["moddijet"].push_back(1); //1
  modelorder["moddijet"].push_back(1); //1
  ...
  if (findorder){
    ...
    for (auto themodel : models){
      ...
      w[themodel]->var("mgg")->setMax(std::max(MAXmass,MAXmassBE));
      w[themodel]->Print("V");
 
      //========================================================================
      std::cout << "Adding bkg data" <<std::endl;
      AddBkgData(w[themodel], isample, year, ws_dir, blind, cats, thecat);
      //========================================================================
      ...
      // //We will save all models and orders in a file initially. 
      fout[themodel] = new TFile(Form("%s/bkg_%s_%s_%s.root", ws_dir.c_str(),themodel.c_str(), cats[thecat].c_str(), year.c_str()), "recreate");
      fout[themodel]->cd();
	...
	for (int i=1; i<=order; ++i){
	  ...
	  //In VVDijet only two orders are allowed
...
//-----------------------------------------------------------------------------------
//With fitTo i cannot access offsetting minll. 
// theFitResult theFit(RooAbsPdf *pdf, RooDataSet *data, double *NLL, int *stat_t, int MaxTries, float minMassForFitBkg, float maxMassForFitBkg){
...
//     tmpfit.minNll = nll->getVal();//minnll;
...
theFitResult theFit(RooAbsPdf *pdf, RooDataSet *data, double *NLL, int *stat_t, int MaxTries, float minMassForFitBkg, float maxMassForFitBkg){
  ...
  while (stat!=0){
    ...
    //Including offset 
    //-------------------
    // RooNLLVar *nll=new RooNLLVar("nll","nll",*pdf,*data,RooFit::Range(minMassForFitBkg,maxMassForFitBkg));
    RooAbsReal * nll = pdf->createNLL(*data, RooFit::Range(minMassForFitBkg,maxMassForFitBkg) );
    ...
    // minuit_fitTest->minimize("GSLMultiMin","conjugatebfgs2");//conjugatefr
...
RooAbsPdf* buildPdf(std::string model, std::string name, int catnum, RooRealVar* xvar, RooWorkspace* w, std::vector<std::string> cats){
  ...
  if ( model == "dijet" ){
    ...
    formula = "TMath::Max(1e-50,pow( @0 + (@0*@0)*@1, -1*@2*@2))";
    ...
    formula = "TMath::Max(1e-50,pow( @0 + (@0*@0)*@1 + (@0*@0*@0)*@2, -1*@3*@3))";
    ...
    formula = "TMath::Max(1e-50,pow( @0 + (@0*@0)*@1 + (@0*@0*@0)*@2 + (@0*@0*@0*@0)*@3, -1*@4*@4))";
    ...
    formula = "TMath::Max(1e-50,pow( @0 + (@0*@0)*@1 + (@0*@0*@0)*@2 + (@0*@0*@0*@0)*@3 + (@0*@0*@0*@0*@0)*@4, -1*@5*@5))";
    ...
    //------------------------------------------------------------------------
    //invpow3 model
  } else if ( model == "invpow3") {
    coefList = new RooArgList(*xvar, 
			      *w->var(TString::Format("PhotonsMass_bkg_invpow_slo_cat%d", catnum)), 
			      *w->var(TString::Format("PhotonsMass_bkg_invpow_qua_cat%d", catnum)), 
			      *w->var(TString::Format("PhotonsMass_bkg_invpow_cub_cat%d", catnum)), 
			      // *w->var(TString::Format("PhotonsMass_bkg_invpow_alp_cat%d", catnum)) 
			      *powlinasq
			      );

    formula = "pow(1+@0*@1+@2*@0*@0+@3*@0*@0*@0,@4)";
    ...
    formula = "TMath::Max(1e-50,pow(@0,@1+@2*log(@0))*pow(1.-@0*@3,@4))";
    ...
    formula = "TMath::Max(1e-50,pow(@0,@1+@2*log(@0))*pow(1.-(@3*@0+@4*@0*@0),@5) )";

    std::cout << "====================================================================" << std::endl;
    std::cout << formula << std::endl;

  }else if ( model == "moddijet3" ) {
    coefList = new RooArgList(*xvar, 
			      *w->var(TString::Format("PhotonsMass_bkg_moddijet_lina_cat%d", catnum)), 
			      *w->var(TString::Format("PhotonsMass_bkg_moddijet_loga_cat%d", catnum)), 
			      // *linbsq,
			      *w->var(TString::Format("PhotonsMass_bkg_moddijet_linb_cat%d", catnum)), 
			      *w->var(TString::Format("PhotonsMass_bkg_moddijet_quab_cat%d", catnum)), 
			      *w->var(TString::Format("PhotonsMass_bkg_moddijet_cubb_cat%d", catnum)), 
			      *w->var(TString::Format("PhotonsMass_bkg_moddijet_sqrb_cat%d", catnum)) 
			      ) ;

    w->var(TString::Format("PhotonsMass_bkg_moddijet_sqrb_cat%d", catnum))->setConstant(1); 
    
    formula = "TMath::Max(1e-50,pow(@0,@1+@2*log(@0))*pow(1.-(@3*@0+@4*@0*@0+@5*@0*@0*@0),@6) )";
...
theFitResult BkgModelFitFunc(RooWorkspace* w, std::string model, bool blind, bool dobands, const std::string &year, const std::string &ws_dir, int order, int c, std::vector<std::string> cats, bool findorder, std::vector<int> orderforcategory, RooArgSet* theparamsout) {
  ...
  // minMassFit = minMassForFitBkg;
  // maxMassFit = maxMassForFitBkg;
  ...
  nBinsMass = (int) round( (maxMassFit-minMassFit)/nBinsMassperGeV );
  // RooBinning* mggRooBinning = new RooBinning(nBinsMass, minMassFit, maxMassFit,"mgg");
  // mgg->setBinning(*mggRooBinning);
  mgg->setBins(nBinsMass);
  ...
  if ((model=="invpow") && (order==3)){model="invpow3";}
  ...
  if ((model=="moddijet") && (order==3)){model="moddijet3";}
  ...
  // if ( year == "2018" && model == "Expow" ){ RemoveRangeParams(theparams); }
  // if ( year == "2017" && model == "Expow" && c==1 ){ RemoveRangeParams(theparams); }
  // if ( year == "2018" && model == "Laurent" && c==1 ){ RemoveRangeParams(theparams); }
  ...
  //These are the data in the full range, the data0
  ...
  nBackground_more = new RooRealVar( Form("ftest_pdf_%s_%s%d_norm", cats[c].c_str(), modelnametopdf[model].c_str(), order ) , "nbkg", data->sumEntries(),0,3*data->sumEntries());
  ...
  int fitStatus = 0;
  double thisNll = 0.;
  fitresult = theFit(PhotonsMassBkgTmp0[model], data, &thisNll, &fitStatus, /*max iterations*/ 3, minMassForFitBkg, maxMassForFitBkg) ;
  ...
  if (order == orderforcategory[c]){
    ...
    w->import(*nBackground_more, RecycleConflictNodes());
  ...
  //setlogy only applies inside PlotFitResults due to canvas splitting. It is useless here. 
  // ctmp->SetLogy();
  // ctmp->SaveAs( Form("%s/Bkg_cat%d_%s_%s_log.png", ws_dir.c_str(),c, model.c_str(), year.c_str() ) );
  //So, choose what you want in the function and apply that. 
  // ctmp->SaveAs( Form("%s/Bkg_cat%d_%s%d_%s_log.png", ws_dir.c_str(),c, model.c_str(), order, year.c_str() ) );
  // ctmp->SetLogy(0);
...
gof PlotFitResult(RooWorkspace* w, TCanvas* ctmp, int c, RooRealVar* mgg, RooDataSet* data, std::string model, RooAbsPdf* PhotonsMassBkgTmp0, float minMassFit, float maxMassFit, bool blind, bool dobands, int numoffittedparams, const std::string &year, const std::string &ws_dir, int order){
  ...
  TPaveText* label_chi2 = new TPaveText(0.65,0.11,0.89,0.22, "brNDC");
...
void RemoveRangeParams(const RooArgSet* params) {
  ...
  std::cout << std::endl; std::cout << "Entering RemoveRangeParams" << std::endl;
  TIterator* iter(params->createIterator());
  for (TObject *a = iter->Next(); a != 0; a = iter->Next()) {
    RooRealVar *rrv = dynamic_cast<RooRealVar *>(a);
    if (rrv) { rrv->removeRange(); std::cout << " " << rrv->GetName()<< std::endl; }
  }  
}
