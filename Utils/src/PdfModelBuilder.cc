  ...
  if (order%2==0){
    ...
    for (int i=1; i<=npows; i++){
      ...
      if (i==1) {
        ...
        dependents->add(*params[pname]);
        params.insert(pair<string,RooRealVar*>(pname, new RooRealVar(pname.c_str(),pname.c_str(),TMath::Max(-10.,-2.*(i+1)),0.,1000.)));
...
RooAbsPdf* PdfModelBuilder::getAtlas(string prefix, int order){
  if(order<1 ){
	...
	if( order==1){
    	params.insert(pair<string,RooRealVar*>(coeff1, new RooRealVar(coeff1.c_str(),coeff1.c_str(),-10. ,-500.,500.)));
	}else if(order==2){
    	params.insert(pair<string,RooRealVar*>(coeff1, new RooRealVar(coeff1.c_str(),coeff1.c_str(),35. ,-500.,500.)));
	}else{
    	params.insert(pair<string,RooRealVar*>(coeff1, new RooRealVar(coeff1.c_str(),coeff1.c_str(),-60. ,-500.,500.)));
  	...
  	for (int i=1; i<=order; i++){
		...
		if(i==1 && order==1){
    		params.insert(pair<string,RooRealVar*>(logc, new RooRealVar(logc.c_str(),logc.c_str(),10.,-500.,500.)));
		}else if(i==1 && order==2){
    		params.insert(pair<string,RooRealVar*>(logc, new RooRealVar(logc.c_str(),logc.c_str(),-10.,-500.,500.)));
		}else if(i==2 && order==2){
    		params.insert(pair<string,RooRealVar*>(logc, new RooRealVar(logc.c_str(),logc.c_str(),-2.,-500.,500.)));
		}else if(i==1 && order==3){
    		params.insert(pair<string,RooRealVar*>(logc, new RooRealVar(logc.c_str(),logc.c_str(),60.,-500.,500.)));
		}else if(i==2 && order==3){
    		params.insert(pair<string,RooRealVar*>(logc, new RooRealVar(logc.c_str(),logc.c_str(),30.,-500.,500.)));
		}else if(i==3 && order==3){
    		params.insert(pair<string,RooRealVar*>(logc, new RooRealVar(logc.c_str(),logc.c_str(),3.,-500.,500.)));
		}
		if (i==1){
			formula_exp = Form("@2");
		}
